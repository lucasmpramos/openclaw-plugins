#!/usr/bin/env node
// Create an "Audio Usage Log" database under the B.O.B page
...
if (!parentPageId) {
  console.error('Usage: node create-db.mjs --parent <page_id>');
  process.exit(1);
}

const title = 'Audio Usage Log';
...
const db = await notion('POST', '/databases', {
  ...
  properties: {
    ...
    From: { rich_text: {} },
    MediaPath: { rich_text: {} },
    DurationSec: { number: { format: 'number' } },
    SizeBytes: { number: { format: 'number' } },
    Language: { rich_text: {} },
    Model: { rich_text: {} },
    TranscribeMs: { number: { format: 'number' } },
    Success: { checkbox: {} },
    Error: { rich_text: {} }
  }
});
